...
band pairs fixed in advance. In contrast, we use coupling to construct the
learned token representation itself, with frequency decomposition,
band-pair weighting, and downstream usage optimized jointly with the
encoder rather than treated as a fixed input feature; and we report the
learned gate values for each corpus to quantify where the model retains or
suppresses cross-frequency content. This work addresses the token side of
