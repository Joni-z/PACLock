...
\midrule
TUSZ & AUC-PR & FFCL & $0.545 \pm 0.030$ & $0.639 \pm 0.035$ & $\mathbf{0.698\,(1)}$ \\
CHB-MIT & AUC-PR & TFM-Tokenizer pre & $0.627 \pm 0.025$ & $0.698 \pm 0.055$ & $\mathbf{0.716\,(1)}$ \\
IIIC & $\kappa$ & REVE pre & $0.436 \pm 0.003$ & $\mathbf{0.479 \pm 0.009}$ & $0.447\,(1)$ \\
TUEP & AUROC & EEGPT scratch & $0.786 \pm 0.021$ & $\mathbf{0.810 \pm 0.004}$ & $0.799\,(1)$ \\
TUEV & $\kappa$ & REVE pre & $0.685 \pm 0.039$ & $\mathbf{0.690 \pm 0.036}$ & $0.680\,(1)$ \\
TUAB & bal.\ acc. & ST-Transformer & $\mathbf{0.820 \pm 0.004}$ & $0.816^{(1)}$ & $0.820\,(1)$ \\
Sleep-EDF & $\kappa$ & ContraWR & $\mathbf{0.692 \pm 0.015}$ & $0.642 \pm 0.014$ & $0.639\,(1)$ \\
CAUEEG & bal.\ acc. & BIOT scratch & $\mathbf{0.561 \pm 0.012}$ & $0.525 \pm 0.015$ & $0.511\,(1)$ \\
ISRUC & $\kappa$ & CBraMod pre & $\mathbf{0.754 \pm 0.008}$ & $0.702 \pm 0.009$ & $0.698\,(1)$ \\
ADFD & bal.\ acc. & BIOT scratch & $\mathbf{0.525 \pm 0.021}$ & $0.505 \pm 0.061$ & $0.461\,(1)$ \\
TUAR & $\kappa$ & CBraMod pre & $\mathbf{0.715^{(1)}}$ & $0.620 \pm 0.037$ & $0.600\,(1)$ \\
Siena & AUC-PR & REVE pre & $\mathbf{0.518 \pm 0.117}$ & $0.170 \pm 0.086$ & $0.201\,(1)$ \\
\bottomrule
\end{tabular}}
\end{table}
...
model on the same corpora are $0.055$, $0.036$, $0.035$ and $0.009$; they
...
\subsection{The same token across encoders}
\label{sec:content-structure}

Table~\ref{tab:content-structure} collects the tokenizer's effect across
four model families and five encoder configurations. The interventions are
not the same experiment, and the table says which is which: for the two
CroFreMo encoders it is the duplex tokenizer against a waveform-only one;
for CBraMod it is interaction rows against matched waveform rows added to
the native tokenizer; for LaBraM and REVE it is the whole CroFreMo
tokenizer, band decomposition included, against the host's own. The tri-axial encoder, which attends along a dedicated
...
\centering
\small
\caption{Tokenizer benefit across four model families and five encoder
configurations. The interventions differ and the numbers are not one
quantity: duplex vs.\ waveform-only tokenizer (CroFreMo encoders);
interaction rows vs.\ matched waveform rows added to the native tokenizer
(CBraMod); the whole CroFreMo tokenizer vs.\ the native tokenizer
(LaBraM, REVE). Tri-axial: three seeds; the rest single seeds.}
...
\midrule
Tri-axial CroFreMo & duplex tokenizer & waveform-only tokenizer & $+0.15$ & $-0.03$ & $+0.03$ & $+0.01$ & $0.00$ \\
Axis-free CroFreMo & duplex tokenizer & waveform-only tokenizer & $+0.107$ & $+0.063$ & $+0.136$ & $+0.027$ & --- \\
...
and loses on TUSZ. Within the two CroFreMo encoders, the duplex tokenizer
helps the folded-attention encoder on all four corpora and the
dedicated-frequency-attention encoder on TUEV alone, where the gain
concentrates on periodic epileptiform discharges (GPED $+0.27$, PLED
$+0.09$ in per-class F1, Table~\ref{tab:perclass} in the appendix).
Whether an encoder with a dedicated frequency axis or a spectral branch
computes for itself what the interaction rows supply is a question for
probing or intervention experiments; the table records that the benefit
of explicit cross-frequency content varies with the encoder and the
corpus, not why.
...
Appendix~\ref{app:objective} and finetuned under one fixed recipe, and the
