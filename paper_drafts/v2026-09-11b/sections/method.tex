...
bands. The grid is then processed by one of two encoders: a tri-axial transformer that attends along each axis in turn, or --- the final model --- an axis-free transformer with no frequency sub-layer, whose only explicit cross-frequency input is the interaction token.
...
$\alpha$-weighted sum of Eq.~(\ref{eq:aligned}) nearly cancels. The
weights $\alpha_{ij}$ are likewise computed with the column sum
$\sum_{i'<j}|Z_{i'j}|$ floored at $\epsilon$, so a column of zero coupling
gives $\alpha_{ij} = 0$ and $\mathbf{u}_j = 0$, and band $j$'s interaction
token is then $\mathbf{a}_j$ scaled toward zero rather than undefined.
...
\subsection{Encoders: dedicated frequency attention, or folded attention}
\label{sec:encoder}

The grid $(C \times 2n_b \times P \times d)$ can be processed by an encoder
with a dedicated frequency sub-layer, or by one that folds the band rows into
spatial attention. We build both, with the same tokenizer, the same recipe and the same
mean-pooled linear head, because the difference between them is the
experiment of Section~\ref{sec:content-structure}.

\paragraph{Tri-axial encoder (dedicated frequency attention).}
...
\paragraph{Axis-free encoder (folded attention).}
