...
\item \textbf{A small encoder built around it, with the attribution that
  ...
  fixed, the duplex tokenizer improves on a waveform-only tokenizer by
  $0.136$ on CHB-MIT, $0.107$ on TUEV, $0.063$ on TUSZ and $0.027$ on IIIC
  (Section~\ref{sec:attribution}); a matched control in which each band's row keeps its own analytic
  features but no cross-band alignment recovers at most a fraction of
  that gain, so the gain is the cross-frequency alignment itself.
...
against three-seed baselines; the tokenizer effects are several times the
seed variance of the same recipe on the same corpora, the main-table
margins on the small corpora are not, and the text says which is which.
