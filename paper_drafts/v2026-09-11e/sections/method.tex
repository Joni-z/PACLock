...
bands. The grid is then processed by a transformer with no frequency sub-layer, in which the band rows are folded into spatial attention and the interaction token is the only explicit cross-frequency input.
...
supported by optimization. The full model has 2.7M
parameters.
...
from uniform; the mean-vector approach of \citet{canolty2006high} takes the
first circular moment
\begin{equation}
M_{ij} \;=\; \frac{1}{T}\sum_{t} A_j(t)\,e^{i\phi_i(t)}, \qquad
\mathrm{MVL}_{ij} = |M_{ij}| ,
\label{eq:mvl-classical}
\end{equation}
whose modulus is zero when $A_j$ carries no phase information and grows
with the depth of modulation, and whose argument $\angle M_{ij}$ is the
preferred phase.
...
non-overlapping patches of length $\ell$ --- 0.25\,s in CroFreMo, and the
host's own patch length when the tokenizer is placed in another encoder
(1\,s in CBraMod and LaBraM, 0.9\,s in REVE) --- and the moment is
...
Measured over a 0.25\,s patch, it contains as many cycles of band $i$ as
that band's center frequency allows, which for bands below 4\,Hz is less
than one cycle, so that for the slowest bands $Z_{ij}$ is a within-patch
phase-weighted amplitude covariance rather than a cycle-averaged
modulation index; and, like every phase--amplitude statistic,
...
that token is built: it carries the band's analytic-signal features under the
measured coupling geometry, and nothing that rescales them.
...
\frac{\mathbf{u}_j}{\max(|\mathbf{u}_j|, \epsilon)} ,
\label{eq:rotation}
\end{equation}
with the modulus taken componentwise and $\epsilon = 10^{-6}$. Then
$|\mathbf{h}_j| = |\mathbf{a}_j|$ elementwise wherever $|\mathbf{u}_j| \ge
\epsilon$, and $|\mathbf{h}_j| \le |\mathbf{a}_j|$ otherwise.
\end{proposition}
\begin{proof}
Each component of $\mathbf{u}_j/\max(|\mathbf{u}_j|,\epsilon)$ has unit
modulus when $|\mathbf{u}_j| \ge \epsilon$ and modulus below one otherwise.
\end{proof}

The guard $\epsilon$ is the implementation's; it matters only when the
$\alpha$-weighted sum of Eq.~(\ref{eq:aligned}) nearly cancels. When
the column sum $\sum_{i'<j}|Z_{i'j}|$ falls below $\epsilon$, the weights
$\alpha_{ij}$ fall back to a uniform average over the slower bands'
phase features, so $\mathbf{u}_j$ is defined for every value of the
coupling; in that regime it carries no alignment geometry, and
Proposition~\ref{prop:invariance} does not apply to it.
...
feature that a waveform-only grid does not expose. The matched control of
Section~\ref{sec:attribution}, in which each band's row keeps its own
phase and no cross-band alignment enters, is what separates the envelope
from the alignment.
...
Two facts about the gates follow directly from Eq.~(\ref{eq:duplex}):
at $\boldsymbol{\beta}_b = 0$ the fused row of band $b$ equals its waveform
token $\mathbf{r}_b$ exactly, and at $\gamma_b = 0$ the interaction row of
band $b$ is the zero vector before positional embedding and carries no
coupling content. A zero interaction row still occupies a
...
blocks with no frequency-specific structure. In each block, attention runs first along time
...
is the same encoder without it. We use $n_b{=}8$, $d{=}192$, $N{=}6$, four heads, 2.7M parameters. Section~\ref{sec:design}
...
the extra rows carry. CBraMod's own spectral branch remains in place, so this transplant
measures what the interaction rows add on top of a native spectral
feature rather than in its absence.
