...
Baselines and the tri-axial CroFreMo are three-seed means $\pm$ std; the
axis-free CroFreMo (2.7M, the final model) is a single seed, marked $(1)$.
``pre'' marks a baseline evaluated with its released pretrained weights.
TUAB is scored by balanced accuracy for every model.}
\label{tab:main}
...
larger. Removing the coupling token costs $0.136$ AUC-PR on CHB-MIT,
$0.107$ $\kappa$ on TUEV, $0.063$ AUC-PR on TUSZ and $0.027$ $\kappa$ on
IIIC at $d{=}192$, and the four effects have the same sign and comparable
size at $d{=}256$. For scale, the seed standard deviations of the
tri-axial model on the same corpora are $0.029$, $0.045$, $0.029$ and
$0.008$. The two margins by which the model leads the foundation-model
field are therefore not properties of the band decomposition or of the
folded attention alone; without the interaction token the same encoder
falls to $0.635$ on TUSZ and $0.580$ on CHB-MIT, below the tri-axial model
...
\subsection{Content versus structure}
\label{sec:content-structure}

Table~\ref{tab:content-structure} places the same token inside three
encoders that differ in whether they can compute cross-frequency
relations without it. The tri-axial encoder, which attends along a
frequency axis, gains $0.15$ from the token on TUEV and nothing outside
seed variance on the other eleven corpora (the full three-seed table is in
the appendix); on the two seizure corpora on which the tri-axial model
also leads the field, its margin is made of the band decomposition and
the factorized attention, not of coupling
(Table~\ref{tab:arch}). CBraMod's encoder, which keeps its own rFFT
magnitude branch, gains from the interaction rows relative to the matched
waveform rows on the three event-classification corpora --- $0.045$ on
TUEV, $0.057$ on TUEP, $0.015$ on IIIC --- and loses $0.092$ on TUSZ; the
gain exceeds CBraMod's own tokenizer only on TUEV
(Section~\ref{sec:transplant}). The axis-free encoder gains on all four.

\begin{table}[t]
\centering
\small
\caption{The same coupling token in three encoders: change in the primary
metric when the coupling content is switched on, with the encoder, recipe
and token count held fixed. Tri-axial: three seeds; CBraMod (additive,
vs.\ matched waveform rows) and axis-free: single seeds.}
\label{tab:content-structure}
\begin{tabular}{llcccc}
\toprule
Encoder & Own cross-frequency pathway & TUEV & TUSZ & CHB-MIT & IIIC \\
\midrule
Tri-axial CroFreMo & frequency attention axis & $+0.15$ & $-0.03$ & $+0.03$ & $+0.01$ \\
CBraMod + rows & rFFT magnitude branch & $+0.045$ & $-0.092$ & --- & $+0.015$ \\
Axis-free CroFreMo & none & $+0.107$ & $+0.063$ & $+0.136$ & $+0.027$ \\
\bottomrule
\end{tabular}
\end{table}

The pattern is the one the design predicts. An encoder that can derive a
cross-band relation from waveform rows treats the interaction token as
redundant, and it helps only where the relation is too specific to be
re-derived cheaply --- periodic epileptiform discharges, on which the
tri-axial gain concentrates (GPED $+0.27$, PLED $+0.09$ in per-class F1,
Table~\ref{tab:perclass} in the appendix). An encoder with a spectral
branch is not starved either, and the extra rows cost it sequence length
on the long seizure windows. An encoder with no such pathway has to take
the relation from the token or not at all, and it takes it on every
corpus we measure. Coupling belongs in the token when, and because, it is
absent from the encoder.
...
seizure margin, which is the claim the model is built to support, and
every alternative is reported. Within the tri-axial family, collapsing
the learned filterbank to a single broadband band costs $0.11$ on TUSZ
and $0.42$ on CHB-MIT, and replacing factorized attention by a flat
encoder that mixes electrodes before attention costs $0.22$ and $0.53$:
the band-decomposed grid and attention factorized over it are what let a
small model learn rare-positive seizure windows at all.
...
\subsection{The token inside a published encoder}
...
rows are better on TUEV, TUEP and IIIC and worse on TUSZ, so the effect is
attributable to coupling content rather than to the extra channels. Against
CBraMod's own tokenizer the transplant is a clear gain only on TUEV
($+0.077$, four standard deviations of the native model), a tie on TUEP
and IIIC, and a loss on TUSZ. Adding the same rows to CBraMod's
\emph{released} checkpoint is worse than the checkpoint alone on both
corpora tried (appendix): a pretrained encoder is tuned to its own token
distribution, and inserting eight new channels per electrode disturbs it
more than the coupling content repays. We draw the narrow conclusion the
data support --- the token transfers, its effect is attributable, and it
pays off on epileptiform event classification --- and not the broad one.
...
The tri-axial model pretrained with the objective of
Section~\ref{sec:objective} and finetuned under one fixed recipe improves
on its from-scratch counterpart where labels are scarce or noisy (Siena,
$0.170 \to 0.456$; TUSZ, $+0.014$; Sleep-EDF, $+0.033$) and loses where
labels are plentiful (TUEV, $-0.048$; ADFD, $-0.052$; TUAR, $-0.028$), a
pattern the appendix explains rather than excuses: lr-matched frozen
probes show that the pretrained representation is informative (on TUEV,
$0.65$ against $0.39$ for a random encoder), label-fraction curves show
the harm growing with the number of labels, and clean-transfer controls at
the pretraining patch length reproduce the TUEV loss with the entire
backbone loaded. The coupling reconstruction term helps the frozen
...
worse in finetuning. We report these as findings about a small model with
a strong inductive bias in the clinical label regime, and we note that the
axis-free model has no pretrained row: its block structure does not match
the checkpoint, and Siena --- the one corpus where pretraining changed the
picture --- remains the axis-free model's weakest cell.
