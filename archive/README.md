...
|---|---|
| `runs_conv_tokenizer` | the Conv1d patch tokeniser, before it became a GEMM. A mathematically identical change, verified to rel 8.9e-7, moved an ISRUC result by 0.0276 = **13.4 seed standard deviations** (docs/FINDINGS.md), so nothing here is comparable with anything after it |
| `runs_wrong_recipe` | 15 group-B cells run off their own repo recipe -- CBraMod without gradient clipping and with early stopping on, LaBraM without label smoothing. Every deviation understated a baseline, i.e. flattered us (docs/PROTOCOLS.md) |
