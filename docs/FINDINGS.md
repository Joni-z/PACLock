...
## 5.6 预训练判决(2026-09-02 定稿;取代 08-28 版——那版的探针证据被 lr 混淆)

**探针 lr 事故**:参考线探针(v1/v2/rand)用 lr 1e-3,而 ptF 与 v3 轨迹探针
继承微调配方的 1e-4,线性头欠训。08-28 版据此写的"TUSZ 表示 0.542→0.486
退化"不可用;全部 12 个探针已按 1e-3 重跑。1e-4 那组保留为内部对照。

### 微调判决(11 语料,配方逐一核对一致;Siena 用配方匹配对 0.065→0.468)

scratch 更好:TUSZ CHB-MIT TUEV IIIC CAUEEG ISRUC ADFD;持平:TUEP TUAB
Sleep-EDF;预训练更好:Siena(+0.40,逐被试:PN16 AP 0.03→0.59,PN14
0.02→0.13)。TUEV 微调 lr/3、lr/10 不救(0.610/0.570 vs scratch 0.709)。

### 冻结探针(lr 1e-3 匹配)

| checkpoint | TUSZ PR-AUC | TUEV Kappa |
|---|---|---|
| 随机初始化 | 0.408 | 0.386 |
| v1 欠训 1.6ep | 0.484 | 0.635 |
| v2 欠训 1.6ep | 0.542 | 0.518 |
| **v2 收敛 15.4ep(ptF)** | **0.445** | **0.651** |
| v3(FAME)轨迹 20k/40k/60k/80k/100k | 0.333/0.411/0.384/0.472/0.505 | 0.539/0.550/0.471/0.532/0.564 |

### 结论(每条有对照)

1. **现象**:连续重建预训练练到收敛,在标注充足语料上微调不敌 scratch;
   标注量消融(TUEV)显示伤害随标注量单调增长(5%: −0.02 → 100%: −0.14),
   TUSZ 该曲线被稀有正例抽样方差淹没(单 seed),记为不确定。
2. **机制分语料,两种都存在**:
   * TUEV:收敛表示是**所有探针中最好的**(0.651 > v1 0.635),但微调只比
     冻结高 0.03、远低于 scratch——失败在优化(预训练初始化锁死盆地)。
   * TUSZ:收敛表示**真的退化**(0.542→0.445,低于 v1 欠训)——重建目标
     收敛点偏离任务(Kommineni 2605.26434 的机制)。
3. **耦合项**:对表示的作用分语料——TUEV 上有功(v2 收敛 0.651 vs FAME
   轨迹最高 0.564),TUSZ 上有过(0.445 vs FAME@100k 0.505)。既不能开脱
   也不能定罪;论文如实写"corpus-dependent"。
4. **轨迹形状**(FAME):TUSZ 到 100k 仍在上升(0.33→0.50),TUEV 非单调平台
   于 0.53–0.56;没有"最优早停点"的故事。
5. **低标注例外成立且可归因**(Siena)。
6. 旧 v2 欠训 checkpoint 在 TUSZ 的微调 +0.08 与探针 0.542 一致:欠训 =
   温和正则 + 尚未偏离的表示。

### 未回答(留 future work)
离散码/潜空间目标在本架构上能否打破 1–2:场上证据(LaBraM/EEGPT/TFM 自家
预训练增益)说可能,未测。

## 5.8 patch-200 全量加载对照(2026-09-03;回应"预训练反直觉"这一会议质疑)

背景:5.6 的全部微调都在 patch_len=50 的网格上,而预训练在 200;`checkpoint_exclude`
又把三个 tokenizer 卷积丢掉——所以每个"预训练微调"其实是"预训练编码器 + 随机 tokenizer +
异网格"。若预训练的害处只是这个工件,把网格对齐、全量加载后应当翻盘。

TUEV 对(configs/pretrain_ft/tuev_duplex_p200{,ptF}.yaml,单 seed,20 epoch,同配方):

| 设定 | patch | tokenizer | test κ | best val | val 峰值位置 |
|---|---|---|---|---|---|
| scratch(表内) | 50 | — | 0.7094 | 0.609 | ep 3 |
| ptF(表内,tokenizer 重初始化) | 50 | 丢弃 | 0.5662 | 0.433 | ep 2 |
| scratch@200 | 200 | — | 0.6094 | 0.510 | ep 4 |
| **ptF@200(全量加载,网格对齐)** | 200 | 加载 | **0.5306** | 0.410 | ep 1,随后单调下滑 |

结论:
1. 网格对齐 + 全量加载后预训练仍输 scratch 7.9 点(50 网格上输 14.3 点)。害处不是
   tokenizer 重初始化的工件,5.6 的判决成立:标题维持 token-content 版,预训练作分析章节。
2. ptF@200 的 val 在第 1 epoch 即到峰值随后单调下滑,与 5.6 描述的"TUEV 优化锁死"一致:
   预训练表示本身好(冻结探针 0.651 最高),但微调从该点出发走不到 scratch 能到的盆地。
3. patch 200 本身在 TUEV 上就贵 10 点(0.7094→0.6094):5 s 事件窗只剩 5 个 patch,
...
(+1e-4 对照 12 个)、v3 轨迹 5 点 ×2 语料、标注量 12 run、Siena 逐被试 ×2、
门控值落盘。论文分析节按 5.6 的六条写;标题保持 token-content 版。

# 第六部分(2026-09-03 → 09-05):移植门、耦合消融全账、架构对照

## 6.1 移植门:tokenizer 进 CBraMod,频带当通道

直接移植(前端 16 行均值池化成一个 token)11 语料:TUEV +0.048,CAUEEG 平,ADFD −0.018,Sleep-EDF −0.046,IIIC −0.083,
ISRUC −0.188,TUEP −0.009,TUAB −0.019,Siena 地板,TUSZ/CHB-MIT 撞 wall 无结果。判定:池化把频率轴平均掉,不是 tokenizer 的公平测试。
修法(`band_mode: channels`):每个(电极, 行)作为 CBraMod 的一个通道进 criss-cross 编码器,编码器之后再对行取均值,头形状不变。

| seed 0 | CBraMod 自带 tokenizer 从零(3 seed) | 同前端、耦合关闭(raw) | 修好版、耦合开(duplex) |
|---|---|---|---|
| TUEV κ | 0.564±0.019 | 0.595 | **0.632** |
| TUSZ AUC-PR | 0.482±0.043 | 0.422 | 0.475 |

结论:(1) 耦合开/关差 TUEV +0.037、TUSZ +0.052——增益归因于耦合内容;(2) 移植 vs 原生:TUEV +0.068,TUSZ −0.007(0.17 std,平)。
预注册判据(TUSZ ≥ 0.48)以 0.007 之差未过,但"不伤"成立。论文写法:可迁移、事件提升、发作持平、归因成立,不宣称普遍提升。

## 6.2 耦合消融全账(同编码器,raw → duplex)

三 seed:TUEV +0.17(0.536±0.034 → 0.709;rot2 0.733±0.016);IIIC +0.013(0.466±0.008 → 0.479±0.008);
TUEP 0.810→0.810;ADFD 0.501→0.505;CAUEEG 0.517→0.525;TUAR 0.623→0.620;TUAB 0.884→0.883;Sleep-EDF 0.650→0.653;
ISRUC 0.701→0.712;Siena 0.171±0.13→0.170±0.07。单 seed 待补:CHB-MIT 0.667→0.713(+0.05),TUSZ 0.671→0.633(−0.04)。
读法:对我们自己的编码器,耦合 token 只在 TUEV 决定性、IIIC 边际、其余为零。与 6.1 合看,耦合的增益依赖编码器:
无频率轴的 CBraMod 处处受益,有频率轴的三轴编码器只在事件形态上受益——候选主张:"跨频关系是缺失的表征;
显式 token 或频率轴编码器是两条路,事件形态需要显式"。

tusz_type(同 TUSZ 发作窗口、标签换成五类发作类型):raw 0.121±0.030 vs duplex 0.132±0.070,两者近随机(val 0.41 → test 0.1,
类型按病人聚集,被试不相交下不可学)。设计检验无结论。

## 6.3 架构对照:贡献一的解释

raw 前端、同配方、三 seed。TUSZ:8 频带+三轴 0.671±0.026;nb=1 0.558±0.041;flat(通道折叠 v1,0.90M)0.448±0.048。
CHB-MIT:0.667±0.047;0.245±0.068;0.136±0.016。发作检测的领先来自分频与三轴两者,三轴更关键;CBraMod 从零 0.317 落在 nb1/flat 水平。
门控变体三 seed:TUEV hybrid_gate 0.673±0.043 / fusegate 0.559±0.026(duplex 0.709);TUSZ 0.670 / 0.653(raw 0.671);
CHB-MIT 0.713(2 seed)/ 0.652(raw 0.667)。不优于 duplex,不采用。

## 6.4 TUEV 逐类别:耦合 token 的增益集中在周期性痫样放电

raw 三 seed(重跑,与旧 seed 逐位一致:κ 0.577/0.536/0.495)vs duplex 三 seed(κ 0.709/0.711/0.648),test 29,421 窗,按类 F1(seed 平均):

| 类 | raw F1 | duplex F1 | Δ |
|---|---|---|---|
| GPED(全面周期性放电) | 0.552 | **0.817** | +0.27 |
| PLED(偏侧周期性放电) | 0.517 | **0.609** | +0.09 |
| ARTF | 0.398 | 0.557 | +0.16 |
| BCKG | 0.884 | 0.919 | +0.04 |
| EYEM(眼动) | 0.566 | 0.313 | −0.25 |
| SPSW(棘慢波,567 窗) | 0.156 | 0.000 | −0.16 |

读法:增益集中在 GPED/PLED——慢节律上锁定快活动的周期性放电,正是相位–幅度耦合的教科书形态;眼动是无跨频结构的慢偏转,
duplex 把它让掉了;SPSW 太少两者都学不到。这是"耦合 token 对形态标签决定性"的直接机制证据,比 tusz_type 那个不可学的检验硬。
duplex 三 seed 确认:GPED +0.27、PLED +0.09、EYEM −0.25 稳定。

## 6.5 第 9 波收官(2026-09-06):从零 duplex 全部三 seed

| 语料 | duplex 三 seed | raw 三 seed | 耦合 Δ | 最强 baseline | 对 baseline |
|---|---|---|---|---|---|
| TUEV κ | 0.690±0.029 | 0.536±0.034 | **+0.15** | REVE 0.685±0.032 | 平(+0.004) |
| IIIC κ | 0.479±0.008 | 0.466±0.008 | +0.013 | REVE 0.436 | 赢 +0.04 |
| TUSZ AUC-PR | 0.639±0.029 | 0.671±0.026 | −0.03 | FFCL 0.545 | 赢 +0.09 |
| CHB-MIT AUC-PR | 0.699±0.045 | 0.667±0.047 | +0.03 | TFM-pre 0.627 | 赢 +0.07 |
| TUEP AUROC | 0.810±0.003 | 0.810±0.001 | 0 | EEGPT-scr 0.786 | 赢 +0.02 |
| ADFD BAcc | 0.505±0.050 | 0.501±0.040 | 0 | BIOT-scr 0.525 | 平 |
| CAUEEG BAcc | 0.525±0.012 | 0.517±0.027 | 0 | BIOT-scr 0.561 | 负 −0.04 |
| TUAR κ | 0.620±0.030 | 0.623±0.007 | 0 | CBraMod-pre 0.715 (1) | 负 |
| Sleep-EDF κ | 0.642±0.011 | 0.650±0.002 | 0 | ContraWR 0.692 | 负 −0.05 |
| ISRUC κ | 0.702±0.007 | 0.701±0.025 | 0 | CBraMod-pre 0.754 | 负 −0.05 |
| Siena AUC-PR | 0.170±0.070 | 0.171±0.130 | 0 | REVE 0.518(ptS 0.456) | 负 |
| TUAB | 0.816 (1) | 0.884 AUROC (3) | 0 | ST-T 0.820 | 平(seed 1/2 待) |

主表:4 赢(TUSZ、CHB-MIT、IIIC、TUEP)3 平(TUEV、TUAB、ADFD)5 负。耦合:只在 TUEV 决定性(+0.15,三 seed;逐类别集中在 GPED/PLED),
IIIC 边际,其余为零(TUSZ −0.03、CHB-MIT +0.03 均在 1 std 内)。TUSZ ptS 三 seed 0.653±0.033 vs 从零 0.639±0.029:平。

## 6.6 方向重置与 CF1/CF2(2026-09-06/07)

Zhizhe 判定:耦合 token 在三轴模型里全局没有分量(6.2),"1.6M 赢几十 M"只有证据没有故事;三轴 duplex 基本死了。
重置为:部分一,耦合 token 补进无频率轴的 FM 编码器(CBraMod 里旧的替换式移植作废——同前端耦合关闭比 CBraMod 自带低 0.06,
耦合 +0.052 只是填坑;改为加法:保留 CBraMod 全部,加 8 行额外通道,raw/cpl 两臂同参数量);部分二,无频率轴的 CroFreMo,
让 token 承载跨频信息,并在去除的同时加东西(折叠、耦合强度、宽度)。

CF2 阶段一(单 seed):TUEV v1raw 0.546 / v0 0.658 / v1 0.648 / v2 0.657 / v1d192 0.680(三轴 duplex 0.690);IIIC 0.453 / 0.435 /
0.455 / 0.481 / 0.447(0.479);TUSZ 0.534 / 0.644 / 0.612 / 0.591(0.639);CHB-MIT 0.661 / 0.569 / 0.647 / 0.628(0.699)。
读法:无轴家族里耦合承重(TUEV +0.10~0.13、TUSZ +0.06~0.11),v0 不比折叠差,强度特征在 IIIC 有效,宽度在 TUEV 有效。
定稿候选 v3(折叠 + 强度 + d192),12 语料单 seed 在跑。硬规则:不再补 seed,除非 Zhizhe 指示。

## 6.7 CF1/CF2 收官(2026-09-08/09)

**CF2 定稿 v1d192。** 12 语料单 seed 对三轴 duplex 三 seed:TUSZ +0.059、CHB-MIT +0.018、Siena +0.031、TUAB +0.004、
TUEV −0.010、TUEP −0.011、Sleep-EDF −0.003、ISRUC −0.004、CAUEEG −0.014、TUAR −0.020、IIIC −0.032、ADFD −0.044。
d256(4.85M)在 TUAR/Siena/Sleep-EDF/IIIC 更好、在 TUSZ(−0.067)/CHB-MIT(−0.108)/CAUEEG 更差,不选。
不折叠(v0d192)在发作两格输 0.03/0.07;强度特征在 CHB-MIT 崩(0.513);全叠 v3 在小语料崩(ADFD 0.361、TUAR 0.545)。

**耦合归因(定稿配置,tokenizer_mode=raw 其余逐字相同):** d192:CHB-MIT 0.716/0.580(+0.136)、TUEV 0.680/0.573(+0.107)、
TUSZ 0.698/0.635(+0.063)、IIIC 0.447/0.420(+0.027);d256:TUEV +0.078、TUSZ +0.090、IIIC +0.047、CHB-MIT 0.608/0.583(+0.025)。
修正 6.6 的"CHB-MIT 耦合不发力":那是 d128 的结果(−0.014);定稿配置下它贡献最大。

**CF1 加法移植(从零 CBraMod,单 seed;自带三 seed):** TUEV 0.564/0.596/0.641(耦合对 raw +0.045、对自带 +0.077);
TUEP 0.642/0.593/0.650(+0.057/+0.008);IIIC 0.393/0.381/0.396(+0.015/+0.003);TUSZ 0.482/0.476/0.384(−0.092/−0.098);
CHB-MIT 0.317±0.167/0.341/0.465(耦合对 raw +0.124、对自带 +0.148;自带在此语料极不稳定)。add_cplmean 与 add_cpl 相同(TUEV 0.642、TUSZ 0.379)。
**挂官方权重:** TUEV 0.572/0.623 对自带 0.645;TUAR 0.681/0.588 对 0.715;ISRUC OOM 未跑,不重跑。

**三种编码器 × 耦合开关(全文核心表):**
| 编码器 | 跨频通路 | TUEV | TUSZ | CHB-MIT | IIIC |
|---|---|---|---|---|---|
| 三轴 CroFreMo(3 seed) | 频率注意力轴 | +0.15 | −0.03 | +0.03 | +0.01 |
| CBraMod 加法移植(1 seed) | rfft 谱分支 | +0.045 | −0.092 | +0.124 | +0.015 |
| 无轴 CroFreMo v1d192(1 seed) | 无 | +0.107 | +0.063 | +0.136 | +0.027 |
机制:跨频信息缺失时 token 承重,不缺时冗余。
