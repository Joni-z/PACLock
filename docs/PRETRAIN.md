...
# 附录 E(2026-08-22):rung-1 duplex 预训练 —— 执行记录与负结果

## E.1 执行

| | |
|---|---|
| 作业 | `pt_duplex_base` 44058728,b2 GPU-shared,**h100-80 × 1** |
| 结构 | duplex + rotation + nb8,d128 / depth6,patch_len 200,1.671M 参数 |
| 数据 | 10 语料全池 + **原(未排除)TUEG 切片**,706,817 窗口 |
| 目标 | 掩码幅度重建 + **配对行掩码**(commit 7f12853) |
| 步数 | 60,000(= 1.0 个 epoch,池共 1,922,268 窗口) |
| 耗时 / 成本 | **2:05:06,约 25 SU** |

**h100 比 l40s 便宜一半**:实测 `billing=12, cpu=12, mem=250G`,而 l40s 是
`billing=24, cpu=24, mem=125G`。此前所有预训练都跑在更贵且更慢的卡上。
h100 队列也更空(9 节点 vs l40s 3 节点)。

最终重建 loss:TUAB 0.0320 / TUEV 0.0357 / TUSZ 0.0351 / CHB 0.0382 /
Sleep-EDF 0.0501。对比 1.64M 的 pac base(TUAB 0.0377)与 8.57M 的 pac large
(0.0298):**duplex 在 base 参数量上补掉了 base→large 差距的约 70–75%**。
但这个对比不完全公平 —— duplex 每个可见频带给出两行,重建任务本身更容易。

## E.2 下游为负(详见 FINDINGS 第五部分)

CHB −0.050、TUSZ −0.029、TUEV −0.020、ISRUC −0.017,仅 Sleep-EDF +0.021。

## E.3 由此确定的三件事

1. **数据体量不是瓶颈,不要再往那条轴上砸。**60k × 32 = 1.92M 样本 = 池子的
   **1.0 个 epoch**,而 base 的 loss 在 50k→60k 已完全打平(0.0378→0.0377)。
   large(8.57M)能继续降 20% 且 25k 步就超过 base 跑满 60k —— 说明数据支撑得起
   更大的模型,不是数据不够。**但 large 的下游平均 −0.004(9 格 5 负)**,
   所以容量也不是瓶颈。剩下的只有目标函数。
2. **patch_len 断裂要正视。**线 A 三臂:tokenizer 迁移只在 TUSZ 为正(+0.072),
   CHB −0.067、BCI −0.019;p200 几何本身让 CHB −0.12、BCI −0.08。当前配方
   (预训练 200 / 微调 50)使整个前端不迁移,只有 encoder + sinc + β/α 过去。
   p50 预训练约 100 SU(token 数 ×4),696 SU 的余额现在负担得起;
   前提是把 64 通道的 PhysioNet-MI 移出池子(p50 在 20 通道以上爆内存,
   而它只占采样 0.33%)。
