...
## 13. 耦合归因(定稿配置,2026-09-09)—— 故事闭合

CF2 的耦合关闭对照跑在**定稿配置**上(v1d192 / v1d256,只把 tokenizer_mode 换成 raw,其余逐字相同),单 seed:

| 语料 | d192 耦合开 | d192 耦合关 | Δ | d256 耦合开 | d256 耦合关 | Δ |
|---|---|---|---|---|---|---|
| CHB-MIT (AUC-PR) | **0.716** | 0.580 | **+0.136** | 跑 | 0.583 | — |
| TUEV (κ) | 0.680 | 0.573 | **+0.107** | 0.679 | 0.601 | +0.078 |
| TUSZ (AUC-PR) | **0.698** | 0.635 | **+0.063** | 0.631 | 0.541 | +0.090 |
| IIIC (κ) | 0.447 | 0.420 | +0.027 | 0.465 | 0.418 | +0.047 |

四格全正,两档同向,量级 0.03–0.14(seed 噪声约 0.03)。**修正 09-07 的判断**:当时说"耦合在 CHB-MIT 上不发力"来自 d128 的旧配置(−0.014),
在定稿配置下它是四格里贡献最大的一格。结论:我们对 baseline 的两个大赢(TUSZ +0.15、CHB-MIT +0.09)都建立在耦合 token 上。

与 CF1 的关系:CBraMod 保留自己的 rfft 谱分支,不缺跨频信息,加通道只是稀释(TUSZ −0.09);我们的无轴编码器没有任何跨频通路,
耦合 token 是唯一来源,所以四格全正。两条线互相解释:**跨频信息缺失时 token 承重,不缺时 token 冗余。**

## 14. CF1 结论(2026-09-09):可归因,但净收益只有 TUEV

**加法移植进从零训练的 CBraMod**(变量只有额外行的内容,token 数相同,单 seed;native 为三 seed):
| 语料 | CBraMod 自带 | +8 行波形 | +8 行耦合 | 耦合归因 | 对自带 |
|---|---|---|---|---|---|
| TUEV κ | 0.564±0.019 | 0.596 | 0.641 | +0.045 | **+0.077** |
| TUEP AUROC | 0.642±0.028 | 0.593 | 0.650 | +0.057 | +0.008 |
| IIIC κ | 0.393±0.005 | 0.381 | 0.396 | +0.015 | +0.003 |
| TUSZ AUC-PR | 0.482±0.043 | 0.476 | 0.384 | −0.092 | −0.098 |
CHB-MIT / Sleep-EDF 在 b2 跑。

**挂到官方发布的权重上:失败。** TUEV 0.623 对自带 0.645(−0.022),TUAR 0.588 对 0.715(−0.127);ISRUC 因序列模式显存溢出未跑,
方向已明确,**不重跑**。解释:已发布的编码器是在它自己的 token 分布上预训练的,插入 8 倍的新通道破坏了那个分布,预训练越强破坏越大。

**结论**:耦合对同 token 数的对照在 4 格里 3 格为正(TUEV/TUEP/IIIC),说明起作用的是耦合内容而非"多加通道";
但净收益超过已发布模型的只有 TUEV。部分一因此不能作为标题主张,写成一段有限的支撑:
"同一个耦合 token 换到另一种编码器里仍可归因,并在痫样事件分类上带来提升(TUEV +0.077);在状态类任务上无效或有害。"
与 CF2 的对照解释一致:CBraMod 保留自己的谱分支,不缺跨频信息;无轴模型缺,所以那里四格全正。
