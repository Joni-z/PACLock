...
**交接文档:`docs/HANDOFF.md`** —— 三个集群怎么连、论文怎么推、做过哪些实验、现在在跑什么、下一步决策流程。新接手先读它。
...
**新预训练微调首批(09-12 14:33,单 seed,对三 seed 从零)**:Siena 0.342 vs 0.220 **+0.12**;ADFD 0.466 vs 0.456 持平;
**TUAR 0.498 vs 0.591 −0.09**。仍是老规律——从零弱的语料上拉、从零强的语料掉,**不满足"好的不许掉"的验收标准**。其余九格在跑。
**b2 crofremo 撞墙钟一次**:30 h 到点,续跑任务从 checkpoint_100000 接上(114k → 101.5k),损失约 14k 步 ≈ 3.6 h;续跑链还有一环,能跑完。

## 23. 模型选择的错误与纠正(2026-09-12 19:30)

Zhizhe 指出 TUEV 曾经在 0.7 以上。查证属实,而且比记忆中更强:**`paclock_rot2` —— 纯耦合 tokenizer(`tokenizer_mode=pac_interaction`,
只有 n_b 行交互 token、没有波形行)配三轴编码器,TUEV = 0.7328 ± 0.0161(三 seed,09-03 就跑完了)**,高于 REVE 官方权重 0.685、
高于 CBraMod 官方 0.645,也高于我们后来定稿的无轴模型 0.655。

同一编码器、同一配方,只换 tokenizer(全部三 seed):
| TUEV | 只有波形(rawtok)0.536±0.041 | **只有耦合(rot2)0.733±0.016** | 波形+耦合(duplex)0.690±0.036 | 定稿无轴 0.655±0.037(2) |
这比论文现在写的"duplex 优于 waveform-only"强得多:**耦合单独用比波形单独用高 0.20,也高于两者合用。**

**为什么丢了**:09-06/07 方向重置后,我按 TUSZ/CHB-MIT 的**单 seed** 挑定稿模型;纯耦合 tokenizer 在那两格只有一个 seed
(TUSZ 0.688 好、CHB-MIT 0.510 差),我没有给它补 seed 就淘汰了它。而 CHB-MIT 是全表方差最大的语料(duplex 0.699±0.055,
CBraMod 从零 0.317±0.205),一个 seed 不足以淘汰一个模型。这是选型流程的错误,不是数据的问题。

**纠正**:`configs/coupling_only/*`(12 语料,从存档 config 复原),28 个缺失的 seed 已投 amd(7 个打包节点),约一天出齐。
出齐后按三 seed 重新选定稿模型,不再用单 seed 定夺。

## 24. 按证据设计定稿模型(2026-09-12 夜)

不再从现成变体里挑。把 34 个 TUEV 实验按结构因子分组求平均:
| tokenizer | 纯耦合 0.688(12) > hybrid 0.683(3) > duplex 0.665(13) > fused 0.554(2) > 纯波形 0.551(4) |
| 频率轴 | attention 0.666(22) > none 0.635(12) |
| 频带数 | 16 → 0.707(2) > 8 → 0.652(32) |
| 交互 | rotation 0.661(20) > product 0.646(14) |
现定稿模型 v1d192 三项踩反(duplex、无轴、8 带),所以只有 0.655;而**四个最优因子从未组合过**——最好的 rot2 是"纯耦合+rotation+8带+有轴"0.733,
0.722 那个是"纯耦合+product+16带"。

**新设计 `crofremo_n*`**(纯耦合 tokenizer + rotation + 频率注意力 + d128 起步),五臂 × 四个决策语料 × seed 0:
n1 = 16 带;n2 = 16 带 + deep stem(0.728 那个跑用的);n3 = 24 带;n4 = 16 带 + d192;n5 = 16 带 + 正则(wd 0.05、dropout 0.35、增强)。
胜出者再上 12 语料 × 3 seed 做最终主表。另有正则扫描(r1–r4,7 语料)与纯耦合补 seed 在跑,三者互相独立。
