...
**2026-09-13 数值勘误**：下文归档中的 CBraMod TUEV `.7089/κ .6512`
不能作为当前引用值。[CBraMod v3 Table 13](https://arxiv.org/html/2412.07236v3#A5.T13)
报告平衡准确率 `.6671 ± .0107`、κ `.6772 ± .0096`；排除 TUEV 预训练的
独立变体报告 κ `.6744 ± .0121`。下文 Uni-NTFM 的“highest published”也不能
作为已完成全面检索的排名结论。保留原调研文字供追溯，实际引用使用
`REFERENCE-PANEL-2026-09-13.md` 中带论文版本、指标和协议说明的记录。
...
## 核实更新(2026-08-27,写 Related Work 时逐条查证)

* **LaBraM 监督相位** —— vendor/labram/modeling_vqnsp.py:
  `angle = torch.angle(x_fft)` + rec_angle_loss。"同行都丢相位"是错的;
  正确表述:逐频率 bin 的**边缘**相位,不蕴含频率对上的联合量。
  其 std_norm 对 dim=(1,2,3) 归约 —— 频率维被平掉,1/f 失衡保留。
* **FAME = arXiv:2608.01898**(08-03,Yu et al.):per-band 标准化 +
  等权 band loss,即我们的 C3a,被抢发三周。已改为引用并作对照。
* **Bindra & Panwar arXiv:2606.08583**:抹平 1/f 后睡眠分期掉 >0.42,
  临床异常检测仅 0.07–0.13,MI 几乎不掉;7 个 FM 中 6 个显著依赖非周期
  成分 —— 直接支撑我们的赢/输边界机制。
* **Kommineni arXiv:2605.26434**:重建式 EEG FM 偏向非周期、欠表示高频
  振荡、嵌入编码被试身份 > 任务信息;"常打不过更小的监督模型"与我们
  TUEV scratch>pretrain 现象互证;结尾呼吁的"显式针对高频振荡结构的
  辅助损失"正是 C3b 的空位。
* **TFM IIIC**:135,096 样本、60/20/20 subject-wise、跟 BIOT 预处理
  (16 双极);其发表 kappa 0.4985 与我们协议不可逐点比(见 FINDINGS 5.3)。
* **BIOT Table 2 IIIC 原文数**:SPaRCNet 0.4679 / ContraWR 0.4623 /
  CNN-T 0.4481 / FFCL 0.4704 / ST-T 0.4492 / BIOT 0.4932(scratch)、
  0.4949(6-EEG pretrained)。
