...
3. **§0 的立论仍然成立**:baseline 的作用是证明消融发生在有竞争力的 regime 里。
   外部结果来自各自的原生协议，须先核实 cohort 才能判断是否领先。
   2026-09-13 的 [TUEV 协议审计](TUEV-PROTOCOL-AUDIT-2026-09-13.md)
   确认 PACLock/LaBraM 验证受试者仅重合 9/58，测试窗口数也不同；
   历史数字不能直接支持同一 cohort 上领先全部外部 baseline 的结论。
